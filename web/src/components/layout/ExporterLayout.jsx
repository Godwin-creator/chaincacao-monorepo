...
const navLinkClass = ({ isActive }) =>
  `block px-4 py-2 rounded-lg text-sm font-body transition-colors ${
    isActive
      ? 'bg-gold-premium text-text-dark'
    ...
    <div className="min-h-screen flex bg-cream">
      <aside className="w-64 bg-chain-bg flex flex-col shrink-0">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-gold-link text-xs mt-1">Espace Exportateur</p>
        </div>

        <nav className="flex-1 p-4 space-y-1">
          <NavLink to="/exporter" end className={navLinkClass}>Tableau de bord</NavLink>
          <NavLink to="/exporter/eudr-certificate" className={navLinkClass}>Certificat EUDR</NavLink>
          <NavLink to="/exporter/export-records" className={navLinkClass}>Registre d'exportations</NavLink>
        </nav>
